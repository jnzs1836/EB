...
def unfreeze_fund(username,price,volume,db):
    ...
    result = db.session.execute("update fund_account_user set freezing_money = "+  str(freeze_fund) + " where username ='" + username + "'")
    db.session.commit()
    return True
...
def unfreeze_stock(username,security_number,volume,db):
    ...
    result = db.session.execute("update security_in_account set freezing_amount = "+  str(freeze_security) + " where username ='" + username + "' and security_number='" + security_number + "'")
    db.session.commit()
    return True
